...
*
*      OPEN(22,file='binary.in', status='old')
      READ(*,*)mass0(1),mass0(2),tphysf,tb,kstar(1),kstar(2),z,ecc,
     &		neta,bwind,hewind,alpha1,lambda,
     &		ceflag,tflag,ifflag,wdflag,bhflag,nsflag,mxns,idum,
     & 	pts1,pts2,pts3,
     & 	sigma,beta,xi,acc2,epsnov,eddfac,gamma,
     &  tphys, aj,mass(1),ospin(1), aj,mass(2),ospin(2)

      if(kstar(1).lt.0.or.kstar(2).lt.0)then
*         READ(*,*)tphys
*         READ(*,*)aj,mass(1),ospin(1)
         epoch(1) = tphys - aj
         kstar(1) = ABS(kstar(1))
*         READ(*,*)aj,mass(2),ospin(2)
         epoch(2) = tphys - aj
         kstar(2) = ABS(kstar(2))
      else
*
...
*      OPEN(23,file='binary.dat', status='unknown')
 ...
 30   j = j + 1
      ...
      kw = INT(bcm(j,2))
      kw2 = INT(bcm(j,16))
      WRITE(*,99)bcm(j,1),kw,kw2,bcm(j,4),bcm(j,18),
     &            bcm(j,8),bcm(j,22), 
