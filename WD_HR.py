...
def get_v_and_ev(table):
    ...
    v = ((pml*factor+4*np.cos(table['l']/180*np.pi)-\
          11*np.sin(table['l']/180*np.pi))**2 +\
         (pmb*factor+7.6*np.sin(table['b']/180*np.pi))**2)**0.5
...
def open_synthetic_result():
    ...
    synthetic_result = np.load('WD_HR_synthetic_results.npy')[0]['results']
